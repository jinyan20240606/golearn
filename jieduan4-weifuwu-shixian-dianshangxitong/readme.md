...
#### 后台系统：有3大菜单：商品管理，订单列表，用户信息管理

1. 前端配置各个微服务的baseurl和锻端口号，目前没有使用统一网关给前端用
2. 后台管理系统 - 商品列表
3. 商品详情页联调
4. 新增商品接口联调
5. 商品分类联调
...
- 重点是一些前后端联调bug排查方法：
- **重点bug原因积累**
  - bug1: 结构体零值问题：详见jieduan3-0-1shixian-weifuwu-kuangjia/mxshop_srvs/goods_srv/handler/category.go的CreateCategory方法
    - 原因：Go 结构体初始化时：不给字段赋值 → 自动填 零值（0 /false/ ""），GORM 会把这些 0 /false 当成你要存的值！
    - 解法：使用map 不会自动加不存在的 key！
  - bug2: `func BrandList(ctx *gin.Context) {` 看具体注释
- 重点业务意识
  - 后台系统中分清楚哪些是可以在后台系统中见增删改查的，哪些是只读的不允许后台添加的？
    - 商品管理：全部可以由后台系统添加的，增删改的
    - 订单列表：订单也是都是前台用户自行创建的，后台只查看列表即可
    - 用户信息管理：轮博图管理是后台添加的前台也不能添加
      - 用户列表不能在后台里添加，一般自己注册，不手动添加，后台只查看即可
      - 用户地址和用户留言：都是前台添加的，后台只查看即可
