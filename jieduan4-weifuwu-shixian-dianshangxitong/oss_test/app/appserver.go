...
// 请填写您的AccessKeyId。
var accessKeyId string = "LTAI4G4krmn4gHievvMGTErH"

// 请填写您的AccessKeySecret。
var accessKeySecret string = "01zvzUkvWqntIvDAH2Yneamtlxblom"

// host的格式为 bucketname.endpoint ，请替换为您的真实信息。
var host string = "http://mxshop-files.oss-cn-hangzhou.aliyuncs.com"

// callbackUrl为 上传回调服务器的URL，请将下面的IP和Port配置为您自己的真实信息。
var callbackUrl string = "http://q7qwax2vy3.52http.net"

// 用户上传文件时指定的前缀。
var upload_dir string = "user-dir-prefix/"
var expire_time int64 = 3000 //签名的超时时间
...
// OSS 上传文件成功后，会 POST 请求这个接口，后端在这里做【签名校验】
func handlerRequest(w http.ResponseWriter, r *http.Request) {
	// 1. GET 请求：返回前端上传需要的 policy + signature
	if r.Method == "GET" {
		// 获取 OSS 上传签名（Policy + Signature）
...
// OSS 回调校验通过后，必须返回固定格式给 OSS
func responseSuccess(w http.ResponseWriter) {
	// 响应体 JSON 必须包含 Status":"OK"。其他字段（url、msg 等）可加可不加。
