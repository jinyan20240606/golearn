...
func main() {
	...
	// msg <- "hello1" // 此时：阻塞，因为容量为1，只能放一个值，此时会触发死锁

	data := <-msg   // 从channel中取值
	msg <- "hello1" // 在这就没事，因为已经取走了1个值，此时可以放第二个值
...
// 当无缓冲时的传输解法：若想成功发送和接收，则需要先启动一个协程去接受，然后才能发送，而且必须 发送 和 接收 在不同 goroutine 里，否则一定会阻塞死锁
// 然后用协程的话，无缓冲和有缓冲都支持
func main1() {

	var msg chan string
	// 错误 msg <- "hello" ❌ 发送写在了启动协程之前，主协程直接卡死，子协程永远跑不起来。
	msg = make(chan string, 0) // 无缓冲写法，第二个参数不传默认就是0，
	// ✅ 第一步：先启动协程（让它等着接收）
