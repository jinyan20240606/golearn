...
/**
   编译和运行：go的运行本质上都需要先编译成二进制可执行文件，然后再执行这个文件。Go 提供了两种方式来编译和运行 Go 程序：

  1. 使用 go build 命令编译 Go 程序,并生成一个可执行文件,然后需要手动执行这个可执行文件。
  go build ./hello.go
  2. 使用 go run 命令运行 Go 程序，它依然会编译，生成的临时二进制文件放到系统临时目录，自动帮你执行，执行完不保留文件
  go run ./hello.go
  三个平台的可执行文件格式：
    - Windows
    文件名：xxx.exe
    ...
    不能在 Mac / Linux 上跑
    - macOS
    文件名：xxx（无后缀）---- `./hello 直接运行`
    格式：Mach-O（自己的可执行文件格式）
    不能在 Windows / Linux 上跑
    - Linux
    文件名：xxx（无后缀）
    格式：ELF
    不能在 Windows / Mac 上跑
