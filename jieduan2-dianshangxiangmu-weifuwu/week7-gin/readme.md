...
讲解下为什么abort能终止链和return的区别

跟express的洋葱圈一样，next方法实际是控制游标，来指向下一个中间件，中间件执行完不执行next，也会默认移动游标

- 细节
  - 内部洋葱圈队列的游标是int8类型的，说明首先int8/int16/int32 只是底层存储位数不同，你写代码照样用十进制数字赋值、加减乘除，不用转二进制
    - 调用的abort方法就是直接赋值游标为最大值，中间件链路直接执行结束了
    - 游标最大值：abortIndex = 63，整个请求链（全局中间件 + 路由组中间件 + 业务 handler）总数 ≤ 63，超过直接 panic
  - 为什么用 int8？
    - 省内存：每个 Context 的 index 只占 1 字节（int8），不是 8 字节（int）
    - 范围足够：63 < 127（int8 最大值），完全够用
    - 快速比较：int8 比较 / 赋值更快，更轻量


## 1-14 gin返回html模版机制：设置静态文件路径和html文件

不是自己实现的，内置使用的是html/template三方库
