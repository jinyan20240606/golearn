...
import (
	"database/sql" // Go 官方底层数据库驱动接口，GORM 底层就是用它来实现的
	// "error" // 用于返回错误信息 errors.New("xxx 错误"),GORM 里到处都是 error 类型返回值
	"log"
	"os"
	"time"

	"gorm.io/driver/mysql" // MySQL 驱动适配器,让 GORM 能连接 MySQL,提供 mysql.Open(dsn)
	"gorm.io/gorm"         // GORM 核心包,所有 GORM 功能都在这里：gorm.DB、Create、First、Model、AutoMigrate
	"gorm.io/gorm/logger"  // GORM 日志配置,控制 SQL 打印,慢查询警告,日志级别（静默、info、warn、error）
)

type Product struct {
	gorm.Model // 这个gorm.Model是一个结构体，包含了ID、CreatedAt、UpdatedAt、DeletedAt四个字段
	Code       sql.NullString
	Price      uint
}
