...
func main() {
	...
	// 这种字面量写法：写的字段是对应的数据库里的column名称，如果`MyName       string `gorm:"column:name"``这种写法，所以我们查询的时候要用数据库的字段名称name来查询
	//db.Where("name = ?", "bobby").First(&user) // 问号代表占位符被二参替代

	// 如果想用结构体的字段名称来查询，我们就要用结构体的方式查询----
	//db.Where(&User{MyName:"bobby"}).First(&user)

	// 获取全部匹配的记录--用find方法----- 这个会忽略掉零值
	//db.Where(&User{MyName:"bobby1", Age: 0}).Find(&users)

	// 上面这种写法会有问题，因为gorm默认不会把零值作为查询条件，所以Age:0这个条件就被忽略了，所以我们可以用下面的map方式来查询
