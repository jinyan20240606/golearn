...
2. gorm中定义表结构体时的字段名识别规则
   1. GORM 是靠【字段类型 + 命名规则 + 标签】自动判断：只要是【结构体 / 结构体切片】类型，GORM 一律当成【关联关系】，不生成数据库字段
   2. GORM 的铁律：这 3 种字段，绝对不存数据库
      1. 普通结构体类型（一对一 / 多对一 关联  --->  GORM 看到是结构体 → 识别为【关联】
      2. 结构体切片类型（一对多 / 多对多 关联） ---> GORM 看到是结构体切片 → 识别为【关联】
      3. 带 gorm:"-" 标签的字段（强制忽略）
   3. 当 GORM 扫描结构体时解析逻辑：
      1. 看类型
         1. 是 int/string/bool 等基础类型→ 存库字段
         2. 是 结构体 / 结构体切片 → 关联关系，不存库
      2. 看命名
         1. Category → 找 CategoryID
         2. User → 找 UserID → 自动建立关联，不生成字段
      3. 看标签
         1. gorm:"-" → 忽略
         2. gorm:"type:json" → 按类型存库
3. **GORM 唯一原生支持、并且会存入数据库的 “数组 / 切片”的表结构体字段类型 只有一种：
   1. ✅ []byte → 对应数据库 longblob**
   2. 其他所有切片：[]string，[]int，[]*Struct，GORM 都不支持直接存库！必须用 自定义类型 或 datatypes.JSON 包装
