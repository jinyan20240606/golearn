...
// 定义一个 “消息结构体”，相当于 Go 里的 struct。名字一般叫：XXXRequest = 请求体
// 用来客户端发给服务端的数据
type HelloWorldRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`       // 1是编号 ，不是值，是序列化时的标识 ID
	Age           int32                  `protobuf:"varint,2,opt,name=age,proto3" json:"age,omitempty"`        // int不支持，需要使用int32或64
	Courses       []string               `protobuf:"bytes,3,rep,name=courses,proto3" json:"courses,omitempty"` // 相当于Courses []string, 定义切片类型，列表、数组、多个值 都用 repeated
...
func (x *HelloWorldRequest) GetAge() int32 {
	if x != nil {
		return x.Age
	}
	return 0
...
func (x *HelloWorldRequest) GetCourses() []string {
	if x != nil {
		return x.Courses
	}
	return nil
...
const file_helloworld_proto_rawDesc = "" +
	...
	"\x05Hello\x120\n" +
	"\x05Hello\x12\x12.HelloWorldRequest\x1a\x13.HelloWorldResponseB\x0fZ\r./;helloworldb\x06proto3"
...
var file_helloworld_proto_depIdxs = []int32{
	0, // 0: Hello.Hello:input_type -> HelloWorldRequest
	1, // 1: Hello.Hello:output_type -> HelloWorldResponse
