...
3. 依赖的创建过程太复杂
	工厂方法解决还不太优雅、更好的是ioc容器，就可以完美解决这个问题
4. 扩展性差

如果大家熟悉其他面向对象语言， java， python， 父类和子类， 依赖的是父类，其实这个问题可以得到解决的，
但是在go语言中，没有继承的概念，我们通过接口来解决这个问题， class比较重，接口就轻量很多， 我们重点要知道的是面向对象解决了什么问题， 封装、继承、多态

---go的接口解决了类似面向对象的过度依赖问题，但是没有解决创建的过程的问题

但是没有解决创建过程的问题, 依赖的创建过程太复杂，2种方法解决，就可以解决这个问题：
       1. 工厂方法解决
	   2. ioc容器的话（spring）
...
- DIP(Dependency Inversion Principle) 依赖倒置原则
	...
	- 上层不应该依赖底层模块，他们之间应该通过抽象(父类、接口，接口)来互相依赖，---- 就是面向接口编程
	- 能做到依赖反转：， A->B  --变成-> A->(接口)->B
	这样做的好处：
		1. 符合开闭原则
