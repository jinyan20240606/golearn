...
// 简单工厂模式：直接是对应获取书的工厂，直接通过name传参，得到对应书的结构体示例
// func GetBook(name string) Book { // 该方法直接返回书的通用接口类型
// 	if name == "语文书" {
...
// ----------- 下面讲一个抽象工厂模式：直接对应发书人的工厂（和上面简单工厂的抽象目标不一样，抽象的更高一层） ----------
// person具体指定的是某个类型的人，我现在想抽象出一个角色来，这个角色就叫发书人，发书人这个工厂有获取书的统一方法，后面还能动态扩展其他方法 如获取练习试卷方法
type Person struct{}
